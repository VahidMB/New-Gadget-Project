...
for key, username, url, label in views:
    ...
    fragment = f'<section data-preview="{key}"'+(' hidden' if key != 'personal' else '')+'>'+''.join(str(node) for node in soup.body.contents)+'</section>'
...
controls = '<div class="gp-preview-tabs" role="group" aria-label="انتخاب پنل">'+''.join(f'<button type="button" data-show="{key}" aria-pressed="{str(key == "personal").lower()}">{label}</button>' for key, _, _, label in views)+'</div>'
fragment_path = ROOT.parent / 'gadget-panels-preview.html'
